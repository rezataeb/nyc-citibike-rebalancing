...
DEFAULT_MAX_STOPS), never inherited from an ad-hoc plan_routes.py run.

Session 48 update: this file is now the dashboard's ONLY route source.
data/route.json was retired -- it held a separate copy of the one-truck plan
for the historical "Show route" toggle, generated by the same
build_route_payload() with these same fixed params, so it was byte-identical
to scenario 1 and the dashboard already drew both through one shared layer.
The historical route is now scenario "1" of this file.

That makes the fixed params more load-bearing, not less: scenario 1 is what a
reviewer sees as "the" rebalancing tour, so it must stay reproducible and must
not drift because someone ran plan_routes.py with different flags. The earlier
framing of this as "the Show route toggle and the fleet slider are deliberately
decoupled" described that same guarantee back when two artifacts existed;
with one artifact there is nothing left to decouple, only params to keep fixed.
...
def _notes(capacity: int, max_stops: int) -> str:
    return (
        ...
        f"capacity={capacity}, max_stops={max_stops}), never inherited from "
        "an ad-hoc plan_routes.py run -- scenario 1 is also the dashboard's "
        "historical 'Show route' tour (data/route.json was retired as a "
        "byte-identical duplicate of it), so these params must stay fixed "
        "for that tour to remain reproducible. Same stopgap depot/max-stops assumptions as every "
